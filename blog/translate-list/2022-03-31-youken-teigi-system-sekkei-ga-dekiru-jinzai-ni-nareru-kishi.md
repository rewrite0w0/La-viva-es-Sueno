...
그 때, **발주자 머리 안에 있는 이미지를 명확히**하지 않으면 개발자는 무엇을 만들어야 좋을지 모릅니다.
...
시스템을 구성하는 파츠 구성도 명확해지므로, **팀 안에 역할 분할 분담**도 간단하겠죠.
